...
public class P011 {
	...
	public static long toArray(int gridSize) {
		// creates a scanner to read the string of numbers
		Scanner strS = new Scanner(str);

		int[][] theArray = new int[gridSize][gridSize];
		// fills the array
		for (int row = 0; row < gridSize; row++) {
			for (int col = 0; col < gridSize; col++) {
				theArray[col][row] = Integer.parseInt(strS.next().trim());
			}
		}
